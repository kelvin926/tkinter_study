...
def open_file():
    if os.path.isfile(filename): #파일 있으면 True / 없으면 False
        with open(filename, "r", encoding="utf-8") as file: #파일 열고 자동으로 닫음.
            txt.delete("1.0", END) #텍스트 위젯 기존 본문 삭제
            txt.insert(END, file.read()) #파일 내용을 본문에 입력
    else:
        print("저장된 파일이 없습니다.")

def save_file():
    with open(filename, "w", encoding="utf-8") as file: #파일 열고 자동으로 닫음.
        file.write(txt.get("1.0", END)) #텍스트 위젯 본문을 가져와서 저장.
...
# 본문
txt =Text(root, yscrollcommand=scrollbar.set) #스크롤바와 동기화 (많이쓰면 스크롤바 활성화)
